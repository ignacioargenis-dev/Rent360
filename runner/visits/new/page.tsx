...
export default function NewVisitPage() {
  ...
  const validateStep = (currentStep: number) => {
    ...
    switch (currentStep) {
      case 1:
        if (!formData.propertyId) newErrors.propertyId = 'Debes seleccionar una propiedad';
        if (!formData.clientId) newErrors.clientId = 'Debes seleccionar un cliente';
        break;
      case 2:
        if (!formData.scheduledDate) newErrors.scheduledDate = 'Debes seleccionar una fecha';
        if (!formData.scheduledTime) newErrors.scheduledTime = 'Debes seleccionar una hora';
        if (!formData.estimatedDuration) newErrors.estimatedDuration = 'Debes seleccionar una duración';
        break;
      case 3:
        if (!formData.priority) newErrors.priority = 'Debes seleccionar una prioridad';
        if (formData.earnings <= 0) newErrors.earnings = 'Las ganancias deben ser mayores a 0';
        break;
    }
