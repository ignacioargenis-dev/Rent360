...
export async function GET(request: NextRequest) {
  try {
    ...
    // Parsear los campos JSON para cada propiedad
    const propertiesWithParsedFields = properties.map(property => ({
      ...property,
    ...
    return NextResponse.json({
      properties: propertiesWithParsedFields,
      pagination: {
        page,
        limit,
        total,
...
export async function POST(request: NextRequest) {
  try {
    const user = requireAuth(request);
    requireRole(request, [UserRole.ADMIN, UserRole.OWNER, UserRole.BROKER]);
    ...
    const { error: errorMessage, statusCode, details } = handleError(error);
    return NextResponse.json(
