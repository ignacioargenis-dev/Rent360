...
export async function POST(request: NextRequest) {
  try {
    ...
    // Verificar token de notificación (de DB o env)
    const settings = await db.systemSetting.findMany({
      where: { key: { in: ['khipuNotificationToken'] } }
    })
    const settingsMap = Object.fromEntries(settings.map(s => [s.key, s.value])) as Record<string, string>
    const NOTIFICATION_TOKEN = process.env.KHIPU_NOTIFICATION_TOKEN || settingsMap['khipuNotificationToken']
...
export async function GET(request: NextRequest) {
  try {
    ...
    const settingsMap = Object.fromEntries(settings.map(s => [s.key, s.value])) as Record<string, string>
    const token = process.env.KHIPU_NOTIFICATION_TOKEN || settingsMap['khipuNotificationToken']
