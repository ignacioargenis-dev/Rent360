...
export default function BrokerClientsPage() {
  ...
  const columns = [
    ...
    },
    {
      ...
      label: 'Propiedades',
      sortable: true,
      render: (value: number, client: Client) => (
        <div>
          <div className="font-medium">{value}</div>
          <div className="text-sm text-gray-500">{client.activeContracts} activos</div>
        </div>
      )
    },
    {
    ...
    },
    {
      ...
      render: (value: number) => (
        <div className="flex items-center gap-2">
          <span className="font-medium">{value > 0 ? value.toFixed(1) : 'N/A'}</span>
          {value > 0 && <Star className="w-4 h-4 text-yellow-400 fill-current" />}
        </div>
      )
    },
    {
